...
class Indicators:
    ...
    ################################
    ################################
    # Calculates RSI 14 using historical closes as a pandas data frame.
    # Returns RSI as a float.
    def RSI(historicalCloses: pd) -> float:

        # Creates copy of historical data to be used and modified for this function.
        historicalClosesCopy = historicalCloses.copy()

        # Calculations
        historicalClosesCopy['delta'] = delta = historicalClosesCopy.diff()
        up = delta.clip(lower=0)
        down = -1*delta.clip(upper=0)
        ema_up = up.ewm(com=13, adjust=False).mean()
        ema_down = down.ewm(com=13, adjust=False).mean()
        rs = ema_up/ema_down
        historicalClosesCopy['RSI'] = 100-(100/(1+rs))
        
        # Gets the latest RSI value.
        rsi = float(historicalClosesCopy['RSI'].tail(1))
...
class CandlePatterns:
    ...
    def PinBar(currentCandle: pd) -> string:
        ...
        if currentCandle_C < currentCandle_O:
            ...
            # True if the top wick is more than 50% of the candle size.
